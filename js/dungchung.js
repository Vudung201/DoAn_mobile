...
function checkDangNhapGoogle(email) {
  var password = '123';
  $.ajax({
    url: "php/xulytaikhoan.php",
    type: "post",
        ...
        success: function (data) {
          if (data == "ok") {
            Swal.fire({
              ...
              setListGioHang(null);
              animateCartNumber();
              window.location.href ="http://localhost/DoAn_mobile/index.php";
...
function capNhatThongTinUser() {
  getCurrentUser((data) => {
    if (!data) {
      ...
      document.getElementById("btnTaiKhoan").innerHTML =
        '<i class="fa fa-user"></i> ' + data["Ho"]+" "+data["Ten"];
      document.getElementsByClassName("menuMember")[0].classList.remove("hide");
    }
  });
}

function capNhatThongTinUserGoogle() {
  getCurrentUser((data) => {
    if (!data) {
      document.getElementById("btnTaiKhoan").innerHTML =
        '<i class="fa fa-user"></i> Tài khoản';
